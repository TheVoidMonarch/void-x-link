...
def complete_resumable_upload(client_socket, transfer_id: str) -> Dict:
    ...
    with transfer_locks[transfer_id]:
        try:
            ...
            # Log completion
            log_info(
                f"Completed resumable upload for {
                    transfer_state.filename} ({file_size} bytes) from {
...
def start_resumable_download(client_socket, filename: str, start_position: int = 0) -> Dict:
    ...
    log_info(
        f"Started resumable download for {filename} ({file_size} bytes) from position {start_position}")
