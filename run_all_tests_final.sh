#!/bin/bash
# Script to run all tests for VoidLink
...
echo "Running comprehensive tests..."
./run_comprehensive_tests.sh > test_results/comprehensive_tests.txt
if [ $? -eq 0 ]; then
    echo "Comprehensive tests: PASSED"
    COMPREHENSIVE_RESULT="PASSED"
else
    echo "Comprehensive tests: FAILED"
    COMPREHENSIVE_RESULT="FAILED"
fi
echo ""
...
echo "- Comprehensive Tests: $COMPREHENSIVE_RESULT" >> test_results/final_summary.txt
echo "" >> test_results/final_summary.txt

# Check if all tests passed
if [ "$AUTH_RESULT" = "PASSED" ] && [ "$ENCRYPTION_RESULT" = "PASSED" ] && [ "$FILE_SECURITY_RESULT" = "PASSED" ] && [ "$COMPREHENSIVE_RESULT" = "PASSED" ]; then
