        ...
        if users:
            ...
            for user in users:
                created = datetime.fromtimestamp(
                    user.get("created_at", 0)).strftime("%Y-%m-%d %H:%M:%S")
                last_login = "Never"
                if user.get("last_login"):
                    last_login = datetime.fromtimestamp(
                        user.get("last_login")).strftime("%Y-%m-%d %H:%M:%S")
        ...
        if files:
            ...
            for file in files:
                ...
                uploaded = datetime.fromtimestamp(
                    file.get("timestamp", 0)).strftime("%Y-%m-%d %H:%M:%S")
                security = "Unknown"
                if file.get("security_scan"):
                    security = "Safe" if file.get(
                        "security_scan", {}).get(
                        "is_safe", False) else "Unsafe"
                rows.append([file.get("filename"), size, file.get(
                    "uploaded_by"), uploaded, security])
            print_table(headers, rows)
        else:
...
def cmd_history(args):
    ...
    if history:
        ...
        for message in history:
            time_str = datetime.fromtimestamp(message.get(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="VoidLink CLI - Command Line Interface for VoidLink administration")
